...
- [X] T001 Crear el esqueleto de `backend/src/modules/sistema/` y `backend/src/modules/rrhh/` (`router.py`, `service.py`, `repository.py`, `schemas.py` cada uno), calcado de la estructura de capas ya usada en `caja/` (006) y `finanzas/`/`ventas/` (007).
- [X] T002 [P] Migración Alembic: crear `recuperacion_password` e `intentos_login` (`data-model.md` completo).
- [X] T003 [P] Migración Alembic: crear el trigger `trg_inhabilitar_cuenta_baja_empleado` + función `fn_inhabilitar_cuenta_usuario()` (`data-model.md`, research.md Decisión 4).
- [X] T004 [P] Seed de `role_permisos_modulo`/`role_permisos_tabla`: `Jefe_TI`→`Sistema` (todas las tablas de esta feature), `Jefe_RRHH`→`RRHH` (`empleados`) — primer uso de ambos módulos (`data-model.md`, Extensión RBAC).
- [X] T005 [P] `backend/src/core/security.py`: utilidades de hash de contraseña (`passlib`/bcrypt) y emisión/validación de JWT (solo `usuario_id`, research.md Decisión 1). Incluye generar automáticamente `JWT_SECRET` en `backend/.env` si no existe todavía (parte del setup del entorno de desarrollo) — nunca se le pide al desarrollador que invente o busque este valor a mano; es un secreto propio del proyecto (como las credenciales de MinIO), no una credencial de un tercero como Stripe/SendGrid.
- [X] T006 [P] Frontend: crear páginas vacías `frontend/src/modules/auth/pages/LoginPage.vue`, `RecuperarPasswordPage.vue`, `frontend/src/modules/sistema/pages/UsuariosPage.vue`, `RolesPermisosPage.vue`, `AuditoriaPage.vue`, `frontend/src/modules/rrhh/pages/EmpleadosPage.vue`.

## Phase 2: Foundational (Bloqueante — ninguna user story empieza sin esto)

- [X] T007 [P] Modelos SQLAlchemy: `RecuperacionPassword`, `IntentoLogin` en `models/`; exponer `Empleado`/`Usuario`/`RolePermisoModulo`/`RolePermisoTabla` (ya existentes como modelos de 001) a través de repositorios propios de esta feature.
- [X] T008 Middleware/dependencia FastAPI de autenticación: valida JWT, resuelve `usuario_id` → cuenta/rol/permisos vigentes en cada request (depende de T005) — reemplaza el token de prueba de 001-007 en todos los routers existentes.
- [X] T009 Montar `sistema.router` y `rrhh.router` en `main.py`; aplicar el middleware de T008 a todos los routers de 001-007.
...
- [X] T010 [P] [US1] Unit test: rechazo de login sin distinguir username inexistente de contraseña incorrecta (FR-002, Edge Case).
- [X] T011 [P] [US1] Unit test: rechazo de login de una cuenta con `activo=false` (FR-003).
- [X] T012 [US1] Contract test: `POST auth/login`.
- [X] T013 [US1] Integration test: login exitoso → `ultimo_login` actualizado → `intentos_login` registra el intento (Escenario 1 de `quickstart.md`).

### Implementación de User Story 1

- [X] T014 [US1] `SistemaRepository`: consulta de `usuarios` por `username`, verificación de `password_hash` (bcrypt).
- [X] T015 [US1] `SistemaService.login()`: orquesta T014 + T010/T011, actualiza `ultimo_login`, registra `intentos_login` (depende de T007).
- [X] T016 [US1] Endpoint: `POST auth/login` — `contracts/auth-administracion-sistema.md`.
- [X] T017 [US1] `frontend/.../LoginPage.vue` y guard de navegación que exige JWT válido para toda ruta fuera de `/auth`.
...
- [X] T018 [P] [US2] Unit test: rechazo de una segunda cuenta para un empleado que ya tiene una (FR-008, Edge Case).
- [X] T019 [US2] Integration test: alta de empleado → alta de cuenta → login exitoso con el rol asignado (Escenario 2 de `quickstart.md`).

### Implementación de User Story 2

- [X] T020 [US2] `RRHHRepository`/`RRHHService`: CRUD base de `empleados` (alta, actualización).
- [X] T021 [US2] `SistemaRepository`/`SistemaService.crear_cuenta_usuario()`: INSERT de `usuarios`, hash de `password_inicial` (depende de T018).
- [X] T022 [US2] Endpoints: `POST rrhh/empleados`, `PATCH rrhh/empleados/{id}`, `POST sistema/usuarios` — `contracts/auth-administracion-sistema.md`.
- [X] T023 [US2] `frontend/.../EmpleadosPage.vue` y `UsuariosPage.vue`: alta de empleado y de cuenta.
...
- [X] T024 [P] [US3] Unit test: rechazo de un token de recuperación ya usado o vencido (FR-010, Edge Case).
- [X] T025 [US3] Integration test: solicitar recuperación → confirmar con el token → login con la nueva contraseña → reintentar el mismo token (rechazado) (Escenario 3 de `quickstart.md`).

### Implementación de User Story 3

- [X] T026 [US3] `SistemaRepository`: INSERT de `recuperacion_password` (token aleatorio, vigencia limitada); UPDATE `usado=true` al confirmar (depende de T024).
- [X] T027 [US3] `SistemaService.solicitar_recuperacion()`: envía el enlace por SendGrid (respuesta genérica exista o no el email — mismo criterio anti-enumeración que FR-002).
- [X] T028 [US3] `SistemaService.confirmar_recuperacion()` / `.cambiar_password_propia()`: actualiza `password_hash` (depende de T026).
- [X] T029 [US3] Endpoints: `POST auth/recuperar-password`, `POST auth/recuperar-password/confirmar`, `PATCH auth/mi-password`.
- [X] T030 [US3] `frontend/.../RecuperarPasswordPage.vue`.
...
- [X] T031 [P] [US4] Unit test: un cambio de rol (T033) se refleja en el siguiente request sin nuevo login (research.md Decisión 1, FR-012).
- [X] T032 [US4] Integration test: cambio de rol → efecto inmediato; ajuste de permiso de tabla sin acceso previo al módulo → rechazado (Escenario 4 de `quickstart.md`).

### Implementación de User Story 4

- [X] T033 [US4] `SistemaService.asignar_rol()`: UPDATE de `usuarios.role_id` (depende de T031).
- [X] T034 [US4] `SistemaRepository`: upsert de `role_permisos_modulo`/`role_permisos_tabla` (la FK compuesta ya garantiza la integridad del Edge Case).
- [X] T035 [US4] Endpoints: `PATCH sistema/usuarios/{id}/rol`, `GET`/`PUT sistema/roles/{id}/permisos-modulo/{id}`, `GET`/`PUT sistema/roles/{id}/permisos-tabla/{modulo}/{tabla}`.
- [X] T036 [US4] `frontend/.../RolesPermisosPage.vue`: asignación de rol por cuenta, matriz de permisos de módulo/tabla por rol.
...
- [X] T037 [P] [US5] Unit test: el trigger de T003 inhabilita la cuenta al dar de baja al empleado, y no la reactiva automáticamente al reactivar al empleado (FR-014, FR-015, Edge Case).
- [X] T038 [US5] Integration test: baja de empleado → cuenta inhabilitada → login rechazado → reporte mensual de auditoría refleja los eventos del periodo (Escenario 5 de `quickstart.md`).

### Implementación de User Story 5

- [X] T039 [US5] `RRHHService.dar_baja_empleado()`: UPDATE `empleados.activo=false`/`fecha_baja` (el trigger de T003 hace el resto, depende de T037).
- [X] T040 [US5] `SistemaRepository.reporte_auditoria_mensual(mes, anio)`: agrega `intentos_login` (éxitos/fallos) + `auditoria_log` (acciones) agrupado por usuario.
- [X] T041 [US5] Endpoints: `PATCH rrhh/empleados/{id}/baja`, `GET sistema/auditoria/reporte-mensual`.
- [X] T042 [US5] `frontend/.../AuditoriaPage.vue`: reporte mensual agrupado por usuario.
