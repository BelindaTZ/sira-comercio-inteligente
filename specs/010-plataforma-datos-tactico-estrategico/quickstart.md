...
1. Como `Jefe_TI`, `POST /api/v1/ti/plataforma-datos/modelo` para registrar `fact_venta` y sus dimensiones (`dim_cliente`, `dim_producto`, `dim_tienda`, `dim_caja`, `dim_empleado`), cada una con su `tabla_origen_postgres`.
2. `GET /api/v1/ti/plataforma-datos/modelo` y verificar que las cuatro áreas de OT-7.1 (venta, cliente, inventario, caja) están cubiertas.
...
5. `GET /api/v1/ti/plataforma-datos/corridas?entidad_id=1` y verificar que muestra filas cargadas, filas con error, duración y resultado de cada corrida (FR-006).
...
2. `GET /api/v1/ti/plataforma-datos/corridas/{corrida_id}/calidad` y verificar que ese registro aparece señalado con su descripción del problema.
...
1. Como `Jefe_TI`, `POST /api/v1/ti/plataforma-datos/politica` con el texto inicial.
2. `GET /api/v1/ti/plataforma-datos/politica` y verificar que devuelve ese texto.
