...
- **Campos clave**: `household_id` (PK, natural del dataset), `nombre`, `email` (UNIQUE), `telefono`, `fecha_nacimiento`, `fecha_registro`, `activo`, `consentimiento_datos` (BOOLEAN NOT NULL, default `true` para filas sembradas — grandfathering), `fecha_consentimiento_datos` (TIMESTAMP NOT NULL), `documento_identidad` (VARCHAR(13), NULLABLE, UNIQUE cuando no es NULL — índice único parcial `WHERE documento_identidad IS NOT NULL`, mismo patrón ya usado en el proyecto para campos opcionales-pero-únicos-cuando-existen; Ronda 5).
